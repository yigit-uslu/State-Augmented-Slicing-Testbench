...
def make_parser():
    ...
    parser.add_argument('--experiment_tag', type=str, default='DEBUG Objective & rate constraint = 1.2 & latency constraint l_max = 0.08 unweighted', help = 'Append a custom description to the experiment (time) tag')
    ...
    parser.add_argument('--chkpt_step', type=int, default = 20, help = 'Set checkpoints to save and plot training status.')
    # parser.add_argument('--dual_transform', type=str, default='none', help='If not None, transform dual multipliers before feeding to the primal policy. Currently only softmax is implemented.')

    # Define constraint fncs
    parser.add_argument('--constraints', type=str, default = ' mean-rate ; Slice.HT , mean-latency ; Slice.LL', help = '(comma-separated) list of (semicolon-separated) constraint name and eval slice pairs.')
    parser.add_argument('--obj', type=str, default=' mean-rate , Slice.BE ')

    # Weight the objective and constraint terms
    parser.add_argument('--w_obj', type=float, default=1., help = 'Weight of the objective term.')
    ...
    parser.add_argument('--slice_reassignment_prob', type=float, default = 0.2, help = 'Probability of slice reassignment')
    ...
    parser.add_argument('--T_slices', type=str, default='100, 200', help = 'Number of slicing windows')
    ...
    parser.add_argument('--num_samples', type=str, default='1, 1', help='Number of train/test samples')
    ...
    parser.add_argument('--n_train_epochs', type=int, default = 100, help = 'Number of training epochs')
    parser.add_argument('--r_mean', type=float, default=1.2, help = 'Mean rate constraint')
    parser.add_argument('--r_min', type=float, default=1.2, help = 'Min. rate constraint')
    parser.add_argument('--l_mean', type=float, default=0.08, help = 'Mean latency constraint')
    parser.add_argument('--l_max', type=float, default=0.08, help = 'Max. latency constraint')
    parser.add_argument('--lr_primal', type=float, default=1e-3, help = 'Learning rate for primal model parameters')
    parser.add_argument('--lr_dual', type=str, default='0.1, 0.1', help = 'Learning rates for dual variables during train/test phases')
    ...
    # GNN parameters
    parser.add_argument('--num_features_list', type=str, default = '64', help = 'Number of model features in different layers')
    ...
    parser.add_argument('--n_primal_iters', type=int, default=2, help = 'Number of primal iterations per epoch')
    ...
    parser.add_argument('--dual_test_init_strategy', type=str, default = 'zeros', choices=['zeros', 'mean-regression', 'greedy-regression'], help = 'Initialize test dual multipliers as either all zero, or by regressing against means of learned dual multiplier training distributions or by regressing against instantaneous optimal dual multipliers.')
    ...
    parser.add_argument('--lr_primal_decay_period', type=int, default=50, help='Decay primal learning rate every $lr_primal_decay_period$ epochs')
