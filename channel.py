...
# def plot_tx_rx_locs(tx_locs, rx_locs, associations, save_path = None):
...
def create_channel_matrix_over_time(m, n, T, R, **kwargs):
    resample_rx_locs = kwargs.get('resample_rx_locs', False)
    shadowing = kwargs.get('shadowing', 7) # shadowing standard deviation
    speed = kwargs.get('speed', 1.) # receiver speed (m/s) # 1.0
    ...
    T_range = T if resample_rx_locs else 1
    H_l = np.zeros((m, n, T_range))
    for t in range(T_range):
        ...
        while True:
            ...
            H_l_t = np.sqrt(np.power(10, -L / 10)) # large-scale fading matrix


            associations = (H_l_t == np.max(H_l_t, axis=0, keepdims=True))
