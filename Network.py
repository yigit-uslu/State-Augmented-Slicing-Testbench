...
class Network:
    ...
    def __init__(self, config):
        ...
        self.channel_data_history = [{'t_slice': 0, 'channel_data': self.channel_data}]
    ...
    def step(self, p, t_slice):
        ...
        # Appending the slicing metrics might be draining the RAM.
        if len(self.slicing_metrics):
            self.slicing_metrics[-1] = slicing_metrics
        else:
        ...
        if self.channel_data_history is None or (t_slice + 1) >= len(self.channel_data_history):
            self.channel_data = self.create_channel_data(t_slice = t_slice + 1)
            self.channel_data_history.append({'t_slice': t_slice + 1, 'channel_data': self.channel_data})
        else:
            self.channel_data = self.channel_data_history[t_slice + 1]['channel_data']
    ...
    def simulate_round_robin_scheduling(self, p, t):
        ...
        su_throughput = 8e-6/(substep_length * T_scheduling_rounds) * torch.tensor(total_tx_bytes, dtype=torch.float32).to(device) # Mbits/s
...
class Client:
    def __init__(self, config, network_idx, client_idx):
        ...
        if self.traffic_model.history is None or not len(self.traffic_model.history):
            self.traffic_model.history.append({
                'data_rate_mbps': self.traffic_model.data_rate_mbps,
                'slice.value': self.traffic_model.slice.value
            })
    ...
    def step(self, t_slice):
        ...
        if False:
            ...
            if self.traffic_model.history is None or t_slice >= len(self.traffic_model.history):
                self.traffic_model.step(t_slice)
                self.traffic_model.history.append({
                    'data_rate_mbps': self.traffic_model.data_rate_mbps,
                    'slice.value': self.traffic_model.slice.value
                ...
                # print('self.traffic_model.history: ', self.traffic_model.history)
                self.traffic_model.data_rate_mbps = self.traffic_model.history[t_slice]['data_rate_mbps']
                self.traffic_model.slice = Slice(self.traffic_model.history[t_slice]['slice.value'])
