...
SA_traffic_config = namedtuple('SA_traffic_config', ['data_rate_mbps', 'data_rate_random_walk_low', 'data_rate_random_walk_high', 'data_rate_random_walk_sigma', 'slice', 'slice_transition_mtrx', 'history', 'traffic_data_save_load_path'])
...
def my_indicator_fnc(value, thresh):
    ...
    '''
    indicator = (value > thresh).float() - torch.sigmoid(value - thresh).detach() + torch.sigmoid(value - thresh)
    return indicator
...
def sample_slice_weights(args):
    ...
    slice_reassignment_prob = args.slice_reassignment_prob if args.constraint_type not in ['clientwise'] else 0.
        ...
        for network_idx in range(args.num_samples[phase]):
            ...
            for client_idx in range(args.n):
                ...
                traffic_config = SA_traffic_config(data_rate_mbps=data_rate_mbps,
                                                    data_rate_random_walk_low=[t[0] for t in args.data_rate_mbps],
                                                    data_rate_random_walk_high=[t[-1] for t in args.data_rate_mbps],
                                                    data_rate_random_walk_sigma=data_rate_random_walk_sigma,
                                                    slice=slice,
                                                    slice_transition_mtrx=slice_transition_mtrx,
                                                    history=[{'data_rate_mbps': data_rate_mbps, 'slice.value': slice.value}],
...
def make_eval_fnc(eval_type, eval_slices, args):
    ...
    if eval_type == 'obj-mean-rate':
        def eval_fnc(metrics, slices):
            ...
            return args.w_obj * (-mean_rate / (args.BW_total * args.r_mean))
        
        def metric_fnc(slack):
            return args.r_mean * -1 * (slack / args.w_obj)
        metric_fnc.__name__ = '(bps/Hz)'
        ...
        def eval_fnc(metrics, slices):
            ...
            return args.w_obj * (-min_rate / (args.BW_total * args.r_min))
        ...
        def eval_fnc(metrics, slices):
            ...
            return args.w_constraint_rate * (1 - mean_rate/(args.BW_total * args.r_mean))
        
        def metric_fnc(slack):
            return args.r_mean * (1 - slack / args.w_constraint_rate) # rate in Mbps
        metric_fnc.__name__ = '(bps/Hz)'
        metric_fnc.tol = args.r_mean
        ...
        def eval_fnc(metrics, slices):
            ...
            return args.w_constraint_rate * (1 - min_rate/(args.BW_total * args.r_min))
        
        def metric_fnc(slack):
            return args.r_min * (1 - slack / args.w_constraint_rate) # rate in Mbps
        metric_fnc.__name__ = '(bps/Hz)'
        metric_fnc.tol = args.r_min
        ...
        metric_fnc.__name__ = '(ms)'
        metric_fnc.tol = 1000 * args.l_mean
        ...
        metric_fnc.__name__ = '(ms)'
        metric_fnc.tol = 1000 * args.l_max


    elif eval_type == 'constraint-rate-violation-rate':
        def eval_fnc(metrics, slices):
            active_slice_idx = [i for i,slice in enumerate(slices) if slice in eval_slices]
            if active_slice_idx:
                rate_violation_indicator = my_indicator_fnc(metrics.rate[active_slice_idx], thresh=args.r_min * args.BW_total)
                # print('rate_violation_indicator: ', rate_violation_indicator)
                rate_violation_rate = 1. - torch.mean(rate_violation_indicator)
            else:
                rate_violation_rate = torch.zeros_like(metrics.rate[0])
            return args.w_constraint_rate_violation_rate * (rate_violation_rate - args.r_min_violation_rate)
        
        def metric_fnc(slack):
            return 100 * (slack / args.w_constraint_rate_violation_rate + args.r_min_violation_rate)
        metric_fnc.__name__ = '(%)'
        metric_fnc.tol = 100 * args.r_min_violation_rate


    elif eval_type == 'constraint-latency-violation-rate':
        def eval_fnc(metrics, slices):
            active_slice_idx = [i for i,slice in enumerate(slices) if slice in eval_slices]
            if active_slice_idx:
                latency_violation_indicator = my_indicator_fnc(metrics.latency_quantile[active_slice_idx], thresh=args.l_max)
                latency_violation_rate = torch.mean(latency_violation_indicator)
            else:
                latency_violation_rate = torch.zeros_like(metrics.latency_quantile[0])
            return args.w_constraint_latency_violation_rate * (latency_violation_rate - args.l_max_violation_rate)
        
        def metric_fnc(slack):
            return 100 * (slack / args.w_constraint_latency_violation_rate + args.l_max_violation_rate)
        metric_fnc.__name__ = '(%)'
        metric_fnc.tol = 100 * args.l_max_violation_rate
    ...
    eval_fnc.eval_slices = eval_slices

    return eval_fnc


def make_feature_extractor(slice_features, args):

    if args.constraint_type == 'slicewise':
        ...
        def get_slice_features(channel_data, traffic_data):
            ...
            for cdata, tdata in zip(channel_data, traffic_data):
                ...
                # added normalization
                slice_avg_data_rate_mbps = slice_avg_data_rate_mbps / slice_avg_data_rate_mbps.sum()
    ...
    elif args.constraint_type == 'clientwise':
...
def make_dual_multiplier_sampler(constraint_type, args):
    ...
    elif constraint_type == 'constraint-rate-violation-rate':
        def dual_multiplier_sampler(size):
            lambdas = 5 * torch.rand(size = size, dtype= torch.float32)
            return lambdas 
        
    elif constraint_type == 'constraint-latency-violation-rate':
        def dual_multiplier_sampler(size):
            lambdas = 5 * torch.rand(size = size, dtype= torch.float32)
            return lambdas 
...
def make_constraint_fncs_and_lambda_samplers(args):
    if args.constraint_type == 'slicewise':
        constraints = []
        lambda_samplers = []
        if args.constraint_mean_rate:
            constraints.append(make_eval_fnc(eval_type = 'constraint-mean-rate', eval_slices = [Slice.HT], args=args))
            lambda_samplers.append(make_dual_multiplier_sampler('constraint-mean-rate', args))
        if args.constraint_min_rate:
            constraints.append(make_eval_fnc(eval_type = 'constraint-min-rate', eval_slices = [Slice.HT], args=args))
            lambda_samplers.append(make_dual_multiplier_sampler('constraint-min-rate', args))
        if args.constraint_mean_latency:
            constraints.append(make_eval_fnc(eval_type = 'constraint-mean-latency', eval_slices = [Slice.LL], args=args))
            lambda_samplers.append(make_dual_multiplier_sampler('constraint-mean-latency', args))
        if args.constraint_max_latency:
            constraints.append(make_eval_fnc(eval_type = 'constraint-max-latency', eval_slices = [Slice.LL], args=args))
            lambda_samplers.append(make_dual_multiplier_sampler('constraint-max-latency', args))
        if args.constraint_rate_violation_rate:
            constraints.append(make_eval_fnc(eval_type = 'constraint-rate-violation-rate', eval_slices = [Slice.HT], args=args))
            lambda_samplers.append(make_dual_multiplier_sampler('constraint-rate-violation-rate', args))
        if args.constraint_latency_violation_rate:
            constraints.append(make_eval_fnc(eval_type = 'constraint-latency-violation-rate', eval_slices = [Slice.LL], args=args))
            lambda_samplers.append(make_dual_multiplier_sampler('constraint-latency-violation-rate', args))

    elif args.constraint_type == 'clientwise':
        constraints = [make_eval_fnc(eval_type = 'constraint-clientwise', eval_slices = [Slice.HT, Slice.LL], args=args) for _ in range(args.n)]
        lambda_samplers = [make_dual_multiplier_sampler('constraint_clientwise', eval_slices = [Slice.HT, Slice.LL], args=args) for _ in range(args.n)]

    return constraints, lambda_samplers
